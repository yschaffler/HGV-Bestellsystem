...
// buildStatsPDF generates the statistics PDF from the given rechnungen slice.
// eventTitle is shown as subtitle (e.g. "Event-Auswertung" or "Sommerfest 2025").
// users is the current user list for name resolution.
// catOrder lists category names in display order (may be nil → alphabetical).
func buildStatsPDF(arrAll []Rechnung, eventTitle string, users []User, catOrder []string) (core.Maroto, error) {
	...
	// split into non-storno / storno
	var arrNonStorno, arrStorno []Rechnung
	for _, r := range arrAll {
		if r.Typ == "STORNO" {
			arrStorno = append(arrStorno, r)
		} else {
			arrNonStorno = append(arrNonStorno, r)
		}
	}
	...
	m.AddRows(row.New(5).Add(
		text.NewCol(1, ""),
		text.NewCol(11, eventTitle, props.Text{
			Size: 9, Color: cGray,
		}),
	))
	...
	styleKpiAccent := &props.Cell{BackgroundColor: cDark, BorderType: border.None}
	styleKpiBg := &props.Cell{BackgroundColor: cLight, BorderType: border.None}
	...
	kpiLabelCol := func(w int, s string, sep bool) core.Col {
		st := styleKpiBg
		if sep {
			st = styleSepRight
		}
		return text.NewCol(w, s, props.Text{
			Style: fontstyle.Bold, Size: 7, Align: align.Center, Color: cGray, Top: 3,
		}).WithStyle(st)
	}
	kpiValueCol := func(w int, s string, sep bool) core.Col {
		st := styleKpiBg
		if sep {
			st = styleSepRight
		}
		return text.NewCol(w, s, props.Text{
			Style: fontstyle.Bold, Size: 13, Align: align.Center, Color: cDark, Top: 2,
		}).WithStyle(st)
	}
	kpiSubCol := func(w int, s string, sep bool) core.Col {
		st := styleKpiBg
		if sep {
			st = styleSepRight
		}
	...
	for _, name := range catOrder {
		if prodMap[name] != nil {
			orderedKats = append(orderedKats, name)
			seen[name] = true
		}
	}
	for k := range prodMap {
...
// getStatsForPDF builds the live-data PDF from the current DB state.
func getStatsForPDF() (core.Maroto, error) {
	...
	catOrder := make([]string, len(dbCats))
	for i, c := range dbCats {
		catOrder[i] = c.Name
	}
	return buildStatsPDF(arrAll, "Event-Auswertung", allUsers, catOrder)
}

// getEventPDF builds a PDF from a stored event snapshot.
func getEventPDF(e ArchivedEvent) (core.Maroto, error) {
	allUsers, err := getAllUsers(DB)
	if err != nil {
		return nil, err
	}
	// derive category order from snapshot data (alphabetical as fallback)
	catSeen := make(map[string]bool)
	var catOrder []string
	for _, r := range e.Rechnungen {
		for _, pos := range r.Positionen {
			if pos.Kategorie != "" && !catSeen[pos.Kategorie] {
				catSeen[pos.Kategorie] = true
				catOrder = append(catOrder, pos.Kategorie)
			}
		}
	}
	sort.Strings(catOrder)
	return buildStatsPDF(e.Rechnungen, e.Name, allUsers, catOrder)
}
