...
export const ColorPickerFormat = ({ className, ...props }: ColorPickerFormatProps) => {
  const { hue, saturation, lightness, alpha, setHue, setSaturation, setLightness, setAlpha, mode } = useColorPicker()
  ...
  if (mode === "hex") {
    ...
    useEffect(() => setAlphaInput(String(Math.round(alpha))), [alpha])
    ...
    return (
      <div className={cn("-space-x-px relative flex w-full items-center rounded-md shadow-sm", className)} {...(props as any)}>
        ...
        <div className="relative">
          <Input
            className="h-8 w-[3.25rem] rounded-l-none bg-secondary px-2 text-xs shadow-none"
            value={alphaInput}
            onChange={(e) => {
              setAlphaInput(e.target.value)
              const val = parseFloat(e.target.value)
              if (!isNaN(val) && val >= 0 && val <= 100) setAlpha(val)
            }}
          />
  ...
  if (mode === "rgb") {
    ...
    const [rgbInput, setRgbInput] = useState(rgb.map(String))
    const [alphaInput, setAlphaInput] = useState(String(Math.round(alpha)))

    useEffect(() => setRgbInput(rgb.map(String)), [hue, saturation, lightness])
    useEffect(() => setAlphaInput(String(Math.round(alpha))), [alpha])

    return (
      <div className={cn("-space-x-px flex items-center rounded-md shadow-sm", className)} {...(props as any)}>
        {rgbInput.map((val, index) => (
          <Input
            key={index}
  ...
  if (mode === "hsl") {
    ...
    const [hslInput, setHslInput] = useState(hsl.map(String))
    const [alphaInput, setAlphaInput] = useState(String(Math.round(alpha)))

    useEffect(() => setHslInput(hsl.map(String)), [hue, saturation, lightness])
    useEffect(() => setAlphaInput(String(Math.round(alpha))), [alpha])

    return (
      <div className={cn("-space-x-px flex items-center rounded-md shadow-sm", className)} {...(props as any)}>
        {hslInput.map((val, index) => (
          <Input
            key={index}
            className={cn("h-8 rounded-r-none bg-secondary px-2 text-xs shadow-none", index && "rounded-l-none")}
            value={val}
