    ...
    <div className="flex flex-col gap-3 mt-3">
      <div>
        ...
        <input
          type="text"
          value={form.barName}
          onChange={e => onChange({ ...form, barName: e.target.value })}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <input
            ...
            onChange={e => onChange({ ...form, tableFrom: e.target.value })}
          ...
          <input
            type="number" min={1}
            value={form.tableTo}
            onChange={e => onChange({ ...form, tableTo: e.target.value })}
...
export default function DruckerPage() {
  ...
  function save(settings: PrinterSettings) {
  ...
  function addRule() {
    ...
    const rule = formToRule(newForm, Date.now().toString());
    save({ ...printerSettings, rules: [...printerSettings.rules, rule] });
    setNewForm(emptyForm);
    setShowNewForm(false);
  }

  function startEdit(rule: PrinterRule) {
  ...
  function saveEdit() {
    ...
    const updated = printerSettings.rules.map(r =>
      r.id === editingId ? formToRule(editForm, r.id) : r
    );
    save({ ...printerSettings, rules: updated });
    setEditingId(null);
  }
        ...
        {activeSection === "regeln" && (
          <div className="flex flex-col gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isEditing ? (
                          <div className="flex gap-1 shrink-0">
                            <button onClick={saveEdit} className="p-1.5 text-primary hover:text-primary/80 transition-colors">
                              <Check className="w-4 h-4" />
                            </button>
                            <button onClick={cancelEdit} className="p-1.5 text-muted-foreground hover:text-foreground transition-colors">
                              <X className="w-4 h-4" />
                            </button>
                          </div>
                        ) : (
                          <div className="flex gap-1 shrink-0">
                            <button onClick={() => startEdit(rule)} className="p-1.5 text-muted-foreground hover:text-foreground transition-colors">
                              <Pencil className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => deleteRule(rule.id)} className="p-1.5 text-muted-foreground hover:text-destructive transition-colors">
            ...
            {showNewForm ? (
              <div className="border border-dashed rounded-2xl p-4 bg-muted/5">
                ...
                <RuleForm
                  ...
                  categories={categories}
                  users={users}
                  onToggleCat={toggleNewCat}
                />
                <div className="flex gap-2 mt-3">
                  <Button variant="outline" size="sm" className="flex-1" onClick={() => { setShowNewForm(false); setNewForm(emptyForm); }}>
                    Abbrechen
                  </Button>
            ...
            <p className="text-xs text-muted-foreground/70 leading-relaxed px-1">
              <span className="font-semibold">Tipp:</span> Regeln werden von oben nach unten geprüft — erste Übereinstimmung gewinnt pro Artikel. Zwei Regeln auf denselben Drucker mit unterschiedlichen Kategorien erzeugen zwei separate Bons.
            </p>
          </div>
        )}
